...
const JoinContainer = ({ children, step, setStep }) => {
    ...
    return <main className="text-[#000938] bg-no-repeat bg-cover bg-fixed bg-center" style={{
        ...
        <section className="relative bg-[#f8f8f8]">
            ...
            />
            <div className={` max-w-7xl mx-auto flex flex-col md:flex-row z-10 ${step == 'thanks' || step == 'login' ? 'items-center justify-center' : 'items-start'}`}>
                <div className={`relative h-[calc(100vh-144px)] md:h-[calc(100vh-116px)] overflow-y-auto w-full p-6 md:p-12 z-10  ${step == 'thanks' || step == 'login' ? 'justify-center items-center' : 'md:w-1/2  justify-start'}`}>
                    {
                        showProgressBar && <>
